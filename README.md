...
### Step 1.1 — Capture & Preprocessing

Remove lighting influence from raw photo. Flatten to diffuse color only.

- [ ] Select first test material (recommended: concrete, stone, or fabric)
...
- [ ] Output folder: `output/<material_name>/` with 4 textures + source image
- [ ] File naming convention: `<name>_albedo.png`, `<name>_normal.png`, `<name>_roughness.png`, `<name>_metallic.png`
...
### Step 2.2 — LLM Scene Interpreter (Extension of scene-gen-agent)

- [ ] Extend `prompt_to_scene.py` to accept material metadata as input
- [ ] Design system prompt: LLM generates an interior scene configuration using the given material
